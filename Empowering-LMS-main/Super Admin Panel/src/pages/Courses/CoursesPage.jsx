  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        setLoading(true);
        setError(null);
        // Using /courses/summary endpoint which we updated to support Global courses
        const response = await axios.get(`${API_BASE_URL}/courses/summary`, {
        ...
        if (response.data.success) {
          const coursesData = response.data.data.map((course) => ({
            ...
            author: "Super Admin", // Global courses are by Super Admin
            duration: course.duration || "0h 0m",
            category: course.category || "General",
            level: course.level || "Beginner",
            createdAt: course.createdAt || new Date().toISOString(),
            updatedAt: course.updatedAt || new Date().toISOString(),
            isVisible: course.isVisible !== false, // Default to true if undefined
          }));
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
              <div>
                ...
                <p className="text-gray-600">
                  Manage courses visible to all companies
                </p>
              </div>
                ...
                >
                  ...
                  <p className="text-gray-500 mb-8 max-w-md mx-auto">
                    {searchTerm
                      ...
                      : "Get started by creating your first global course"}
                  </p>
                  {!searchTerm && (
                    <motion.button
                      whileHover={{ scale: 1.05 }}
